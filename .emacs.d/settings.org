...
#+begin_src emacs-lisp
  ...
  (defun isamert/outline-up-heading-or-backward-heading-same-level ()
  ...
  (defun isamert/outline-up-heading-forward-or-forward-heading-same-level ()
  ...
  (evil-define-key 'normal org-mode-map
    ...
    "(" 'isamert/outline-up-heading-or-backward-heading-same-level
    ")" 'isamert/outline-up-heading-forward-or-forward-heading-same-level
...
#+BEGIN_SRC emacs-lisp
  (when (file-directory-p "~/Documents/notes")
    ...
    (setq org-id-link-to-org-use-id 'create-if-interactive-and-no-custom-id) ;; org-store-link creates an ID for header only if called interactively and if there is no custom id
    (setq org-agenda-files `(,life-org ,bullet-org "~/Documents/notes/well.org")) ;; FIXME well.org
    (org-babel-lob-ingest "~/Documents/notes/utils.org")) ;; With this, I can call functions inside this file in other org files
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  (use-package lsp-mode
    ...
    :hook ((web-mode . lsp)
           ...
           (java-mode . lsp))
    :init
    (setq lsp-diagnostic-package nil ;; prefer flycheck
...
#+begin_src emacs-lisp
  (use-package tab-jump-out
    ...
    (define-globalized-minor-mode global-tab-jump-out-mode tab-jump-out-mode (lambda () (tab-jump-out-mode)))
...
#+begin_src emacs-lisp
  ...
  (defun isamert/google-translate-this (from to)
    "Translate selected region or current word with google. Let's you edit the query beforehand. Result is printed into minibuffer."
    (interactive
     `(,(read-string "From: " "en") ,(read-string "To: " "tr")))
    (message
     (shell-command-to-string
      (format
       "curl -s -A 'Mozilla/5.0 (Windows NT 10.0; WOW64; rv:56.0) Gecko/20100101 Firefox/56.0' \"%s\" --data-urlencode 'q=%s' | ggrep -Po '<div dir=\"ltr\" class=\"t0\">\\K[^<]*' | tr -d '\\n'"
       (format "https://translate.google.com/m?sl=%s&tl=%s&ie=UTF-8" from to)
       (read-string
        "Translating: "
        (isamert/region-or 'word))))))
...
#+begin_src emacs-lisp
  ;; TODO: save images under docs/images/screenshot_date_....
...
#+begin_src emacs-lisp
  ...
  (defun well/setup-js ()
    ...
    (interactive)
    (if (string-match "/ui/" (projectile-project-root))
...
#+begin_src emacs-lisp
  (defun well/open-jira-issue-at-point ()
    (interactive)
    (browse-url (format "https://wellco.atlassian.net/browse/%s" (thing-at-point 'symbol))))

  (evil-leader/set-key "oj" 'well/open-jira-issue-at-point)

  (defun well/init-daily ()
    (interactive)
    (find-file "/ssh:mia3:/data/data/com.termux/files/home/storage/shared/Documents/notes/life.org")
    (split-window-right)
    (other-window 1)
    (find-file "/ssh:mia3:/data/data/com.termux/files/home/storage/shared/Documents/notes/bullet.org"))
