...
#+RESULTS:
: isamert/find-all
[[file:settings.org::*org-alert][org-alert]]
** Package repos
...
#+begin_src emacs-lisp
  (when (eq system-type 'darwin)
    ...
    (define-key key-translation-map (kbd "Δ") (kbd "<down>"))
...
#+begin_src emacs-lisp
  ...
  (defun isamert/set-exec-path-from-nix-shell ()
    (interactive)
    (let ((path-from-shell (shell-command-to-string "nix-shell --quiet --run 'echo $PATH'")))
...
#+begin_src emacs-lisp
  ...
  (defun isamert/select-one-of (the-candidates &optional buffer-name)
