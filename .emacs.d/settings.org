...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package which-key-posframe
    ...
    (custom-set-faces
     `(which-key-posframe
       ((t (:background ,(mini-frame-get-background-color))))))
    ;; ^ Calculated this color by calling `mini-frame-get-background-color'
...
#+BEGIN_SRC emacs-lisp
  ...
  ;; TODO: Experiment with not having the following bindings. This frees
  ;; up M-{h,j,k,l} keys so that I can bind anything else. Try to use
  ;; M-a to do window-switching
