...
(use-package eyebrowse
  ...
  (setq eyebrowse-new-workspace t
        eyebrowse-mode-line-style 'current) ;; Only show the current workspace number in modeline. not all the active workspaces
  :config
  (progn
...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package multi-vterm
    ...
    ;;   )
    (isamert/shackle-window "\\*vterm-dedicated\\*" 0.4))
#+END_SRC
           ...
           (selected
            (helm :buffer "*helm-org-headers*"
                  :sources (helm-build-sync-source "headers"
                             :candidates headers
                             :fuzzy-match t)))
           (link-name (read-string "Enter link name: " selected))
           ;; TODO: replace filename:match to filename::match
           (selected-link (concat "[[file:" selected "][" link-name "]]")))
...
#+begin_src emacs-lisp
  ...
  (defun isamert/bullet-create-a-day (date focus)
    (let ((day-entry (isamert/bullet-find-a-day date))
          ...
          (is-last-older nil)
          (result-date nil)
          (given-date (date-to-time (concat date "T00")))
          (content (concat "* [" date "] [/]")))
      (if day-entry
          (goto-char day-entry)
        (progn
          (org-map-entries
           (lambda ()
             (let ((title (org-entry-get nil "ITEM")))
               (save-match-data
                 (when (string-match day-regex title)
                   (let ((current-date (date-to-time (concat (match-string 1 title) "T00"))))
