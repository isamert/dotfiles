...
;; TODO:
;; - Allow parametric directives to appear more than once (@tool=...)
;; - Add @tool=tool_name directive.
;; - Aggreagate :gptel-tools returned by :fn or whatever
...
(defun im-ai-at-point--apply-directive (name info)
  ...
  (if-let* ((props (alist-get name im-ai-at-point--directives nil nil #'equal)))
      (let* (;; 1. run implied directives first
             (info (cl-reduce
                    (lambda (acc implied)
                      (if (eq (car acc) :abort)
                          acc
             ...
             (info (if-let* ((fn (plist-get props :fn)))
                     ...
                     info))
             ;; 4. put :gptel-.. into :gptel-vars
             (info (im-ai-at-point--merge-info
                    info
                    (list :gptel-vars
                      (append (plist-get info :gptel-vars)
                              (cl-loop for (k v) on info by #'cddr
                                       when (and (keywordp k) (and (not (equal ":gptel-vars" (symbol-name k)))
...
(defun im-ai-at-point--merge-info (base override)
  "Merge OVERRIDE plist into BASE plist, override wins."
  (let ((result (copy-sequence base)))
    (cl-loop for (k v) on override by #'cddr
             do (plist-put result k v))
    result))
...
Handles both @name and @name=value forms."
  (let ((directives nil)
        (stripped prompt))
    (cl-loop for (name . _) in im-ai-at-point--directives
             do (cond
                 ...
                 ((string-suffix-p "=" name)
                  (when (string-match (im-ai-at-point--directive-regexp name t) prompt)
                    (push (concat name (match-string 2 prompt)) directives)
                    (setq stripped (s-replace-regexp (im-ai-at-point--directive-regexp name t) " " stripped))))
                 ;; plain @name
                 (t
...
(im-ai-at-point--register "model="
  ...
  :fn (lambda (info)
        (let* ((raw (plist-get info :raw-prompt))
               (match (s-match "@model=\\([^: ]+:[^ ]+\\)\\b" raw)))
          (if-let* ((model-str (cadr match))
                    (entry (assoc model-str (im-ai--gptel-all-models))))
              (list :gptel-backend (cadr entry)
                    :gptel-model   (caddr entry))
            (list :abort (format "Unknown model: %s" (cadr match)))))))
