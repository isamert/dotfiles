...
#+begin_src conf :tangle /sudo::/etc/udev/rules.d/90-backlight.rules
...
#+begin_src conf :tangle ~/.local/share/barrier/.barrier.conf
  ...
  section: options
    ...
    keystroke(alt+BracketL) = switchToScreen(x220)
    keystroke(alt+BracketR) = switchToScreen(trendyol)
  end
#+end_src
