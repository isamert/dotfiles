;;; im-org-complete.el --- Header completion for org-mode  -*- lexical-binding: t; -*-

;; Copyright (C) 2024 Isa Mert Gurbuz

;; Author: Isa Mert Gurbuz <isamertgurbuz@gmail.com>
;; URL: https://github.com/isamert/im-org-complete.el
;; Version: 0.0.1
...
;; (require 'im-org-complete)

;;;; TODO

;; + Provide a way to supply manual completion for parameters
...
(defun im-org-complete-completion-at-point ()
  "Provide a list of completion candidates with documentation.
See `im-org-complete' for detailed documentation."
  (-let ((bounds (bounds-of-thing-at-point 'filename))
         (candidates (im-org-complete)))
    (list
     (or (car bounds) (point))
     (or (cdr bounds) (point))
     ...
     :company-doc-buffer
     (lambda (candidate)
       (with-current-buffer (get-buffer-create "*im-org-complete*")
         (erase-buffer)
...
This function is used by `im-org-complete-completion-at-point'.
  ...
  (-let* ((line (thing-at-point 'line t))
    ...
    (cond
     ...
     ((or (s-blank? what) (looking-back (format " %s" what) (line-beginning-position)))
      (im-org-complete--block-types block-type))
     ((looking-back ":[a-zA-Z0-9_-]+ +\"?" (line-beginning-position))
      (-let* (((start end) (match-data))
        ...
        (im-org-complete--parameter-types what block-type parameter)))
     ((not (s-blank? what))
...
something like `clocktable', `org-ql' etc.  PARAM is the
parameter name, something like `:tangle', `:eval' etc.

Return type is same as described in `im-org-complete'."
  (let* ((param-info
          (alist-get param (im-org-complete--parameters what block-type) nil nil #'equal))
         (param-type (plist-get param-info :type)))
    (--map
     (cons it (list :ann (concat "∈ " param)
                    :doc (im-org-complete--doc-fn what block-type param-type)
                    :type (plist-get param-info :type)))
     (im-org-complete--candify-type param-type))))

(defun im-org-complete--parameters (what block-type)
...
Return type is same as described in `im-org-complete'."
  (let* ((var-candidates (im-org-complete--parameters-from-var what block-type))
         (source-candidates (unless var-candidates
                              (im-org-complete--parameters-from-source-code what block-type)))
         (doc-candidates (unless source-candidates
                           (im-org-complete--get-parameter-completions-from-doc what block-type)))
         (common-candidates (when (equal 'src block-type)
                              (im-org-complete--parameters-from-common-src-args what block-type))))
    (append
     var-candidates
     source-candidates
     doc-candidates
...
(defun im-org-complete--block-types (block-type)
   ...
   (let ((lang (s-chop-prefixes '("org-dblock-write:" "org-babel-execute:") (symbol-name it))))
     (cons
      ...
      (list :ann (concat (symbol-name block-type) " type")
            :doc (im-org-complete--doc-fn lang block-type))))
   (im-org-complete--get-functions-starting-with
    (pcase block-type
...
(defun im-org-complete--parameters-from-common-src-args (what block-type)
  "Simply extract all common parameters from `org-babel-common-header-args-w-values'.

These completions are annotated as \"common\"."
  (im-org-complete--type-completions-for-parameter
   what block-type "common"
   org-babel-common-header-args-w-values))

(defun im-org-complete--parameters-from-source-code (what block-type)
...
These completions are annotated as \"source\"."
  (-let* ((fn-name (im-org-complete--build-fn-name what block-type))
          (fn (-as->
               fn-name %
               (intern %)
               (im-org-complete--get-function-source %)
               (s-lines %)
               (s-join " " %)))
    ...
    (--map
     (cons
      ...
      (list
       ...
       :doc (im-org-complete--doc-fn what block-type "not available. This parameter found in the documentation.")
       :type nil))
     (im-org-complete--extract-parameters-from-source fn params-name))))

(defun im-org-complete--get-parameter-completions-from-doc (what block-type)
    ...
    (--map
     (cons
      ...
      (list
       ...
       :doc (im-org-complete--doc-fn what block-type "not available. This parameter found in source code. See below.")
       :type nil))
...
(defun im-org-complete--parameters-from-var (what block-type)
...
(defun im-org-complete--extract-parameters-from-source (source params-name)
  (-uniq
   (mapcar
    #'cadr
    (append
...
(defun im-org-complete--type-completions-for-parameter (what block-type from parameter)
  (--map
   (cons
    (concat ":" (symbol-name (car it)))
    (list
     :ann (concat what " parameter (" from ")")
     :doc (im-org-complete--doc-fn what block-type (cdr it))
     :type (cdr it)))
   parameter))

;;;; Utils

(defun im-org-complete--get-function-source (function)
...
(defun im-org-complete--get-functions-starting-with (prefix)
...
(defun im-org-complete--get-package-commentary (package)
...
(defun im-org-complete--build-fn-name (what block-type)
  (concat
   (pcase block-type
     ('dblock "org-dblock-write")
     ('src "org-babel-execute"))
...
(defun im-org-complete--candify-type (type)
  (cond
   ((listp type) (-non-nil (mapcar #'im-org-complete--candify-type-1 (car type))))
   (t (when-let ((result (im-org-complete--candify-type-1 type)))
        result))))

(defun im-org-complete--candify-type-1 (type)
  (cond
   ((equal type :any) nil)
   ((symbolp type) (symbol-name type))
   (t (format "%S" (or type "")))))

(defun im-org-complete--stringify-type (type)
  (cond
   ((listp type) (s-join "|" (mapcar #'im-org-complete--stringify-type (car type))))
   ((symbolp type) (symbol-name type))
   (t (format "%s" type))))

(defun im-org-complete--doc-fn (lang block-type &optional type)
  (lambda ()
    (let ((fn-name (concat
      ...
      (concat
       (when type
         (concat "Type is " (im-org-complete--stringify-type type) "\n\n---\n\n"))
       (when-let ((commentary (im-org-complete--get-package-commentary (concat "ob-" lang))))
         (concat commentary "\n\n---\n\n"))
       "⇒ "
       fn-name
       "\n"
...
(provide 'im-org-complete)

;;; im-org-complete.el ends here
