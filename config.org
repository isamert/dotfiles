...
#+begin_src bash :tangle ~/.config/bspwm/bspwmrc
  ...
  export RECT_TOP_POPUP="$((SCREEN_WIDTH / 3 + 100))x400+$((SCREEN_WIDTH / 2 - (SCREEN_WIDTH / 3 + 100) / 2 ))+${TOP_GAP}"
...
#+begin_src conf :tangle ~/.config/sxhkd/sxhkdrc
  ...
  hyper + r; {f, q, e, r, t, v, w, k}
      {firefox, qutebrowser, emacsclient -c, jaro ~, lxtask, vivaldi-stable, whatsapp-nativefier-dark, /bin/sh -c "getpassword --keepass | keepassxc --pw-stdin $PASSWORD_STORE"}
...
#+begin_src bash :tangle ~/.zshrc
  ...
  # This file contains stuff that I don't want to commit to git
  # or stuff that are temporarily there etc.
...
*** Utility functions
The following gets tangled into =~/.zshrc=.

#+name: zsh-utility-functions
#+begin_src bash
...
#+begin_src scheme :tangle ~/.config/associations
  ...
  (assoc
    #:pattern "x-scheme-handler/mpv"
    #:program "which ffmpeg; which mpv; ffmpeg -version; ffmpeg -protocols 2>&1; echo hehehe-$(echo %f | sed 's/mpv:\\/\\///'); vlc $(echo %f | sed 's/mpv:\\/\\///')"
    #:view #t)
